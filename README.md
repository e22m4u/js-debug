...
- Поддержка Node.js (*env*) и браузера (*localStorage*).
- Интерполяция строк со спецификаторами `%v`, `%s`, `%d` и др.
- Поддержка пространства имен и цепочных вызовов.
- Генерация хэшей для отслеживания конкретных вызовов.
- Настройка визуальных отступов для имитации иерархии логов.
- Базовый класс `Debuggable` для наследования функционала логирования.
- Цветной многострочный дамп сложных объектов через метод `inspect`.
